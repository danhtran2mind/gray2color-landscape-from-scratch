...
for path in load_model_paths:
    ...
    if not os.path.exists(path):
        ...
        url_map = {
            "autoencoder": "ckpts/best_model.h5",  # Replace with actual URL
            "unet": "ckpts/unet_colorization_model.keras",  # Replace with actual URL
            "transformer": "ckpts/transformer_colorization_model.keras"  # Replace with actual URL
        }
...
def select_example(input_image_path, output_image_path, model_name):
    # Load the input image
    input_img = Image.open(input_image_path)
    # Load the precomputed output image
    output_img = Image.open(output_image_path)
    # Return the input image, output image, and model name to populate the interface
    return input_img, output_img, model_name
...
with gr.Blocks(theme="soft", css=custom_css) as demo:
    gr.Markdown(
        """
        # Grayscale2Color Landscape from scratch🌄
    ...
    with gr.Row():
        input_img = gr.Image(type="pil", label="Upload Grayscale Landscape", image_mode="L")
        output_img = gr.Image(type="pil", label="Colorized Output")
    ...
    submit_btn.click(
        fn=process_image,
        inputs=[input_img, model_names],
        outputs=output_img
    )
    gr.Examples(
        examples=examples,
        inputs=[input_img, output_img, model_names],
        outputs=[input_img, output_img, model_names],
        fn=select_example,
